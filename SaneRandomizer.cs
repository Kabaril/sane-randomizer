...
public class SaneRandomizer : Mod
{
    ...
    public override void Load()
    {
        ...
        if(Config.LTS22)
        {
            ...
            _randomizer = new Randomizer22(Logger, Config.Seed);
        } else
        {
            _randomizer = new RandomizerDev(Logger, Config.Seed);
        }
    }
    ...
    {
        var minMaxTable = new MinMaxTable(Config);

        //start randomizing
        Logger.Info("Creating Drop Tables");
        DropTable = _randomizer.RandomizeDrops();
        Logger.Info("Creating Trade Tables");
        TradeTable = _randomizer.RandomizeTrades();
        Logger.Info("Creating Item Modification Table");
        ItemModifierTable = _randomizer.RandomizeItemValues(minMaxTable);
        Logger.Info("Creating NPC Modification Table");
        //MUST BE AFTER DROPS
        NPCModifierTable = _randomizer.RandomizeNPCValues(minMaxTable);

...
{
    private static readonly List<int> UseTimeExcluded =
    [
        ItemID.MagicMirror,
        ItemID.BugNet,
    ...
    public override void SetDefaults(Item item)
    {
        ...
        var mod = SaneRandomizer.Instance.ItemModifierTable[item.type];
        if (item.damage != -1) {
            if (config.Damage)
            {
                var damageMod = mod.DamageModifier;
                item.damage = (int)(item.damage * (damageMod / 100f));
            }
        }
        ...
        if (config.UseTime && !UseTimeExcluded.Contains(item.type))
        {
